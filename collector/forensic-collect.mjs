 ...
 *   forensic-collect scan   --case <id> --url <https://worker> --token <ingest> \
 *                           --repos /path/a,/path/b [--key <file|base64>] [--no-content]
 *   forensic-collect repair --case <id> --url <...> --token <...> --repo /path/a
 *   forensic-collect decrypt --case <id> --url <...> --token <...> --id <evidenceId> --key <...>
 */
...
function loadKey(keyArg) {
  if (!keyArg) {
    const env = process.env.FORENSIC_CONTENT_KEY;
    if (!env) return null;
    return b64urlToBuf(env);
  }
  // treat as file path if it exists, else as inline base64url
  try {
    return b64urlToBuf(readFileSync(keyArg, "utf8").trim());
  } catch {
    return b64urlToBuf(keyArg);
  }
}
...
async function cmdScan(a) {
  if (!a.url || !a.token || !a.case || !a.repos) die("scan needs --case --url --token --repos");
  ...
  const { status, data } = await post(agentUrl(a.url, a.case, "/ingest"), a.token, { submissions });
...
async function cmdRepair(a) {
  if (!a.url || !a.token || !a.case || !a.repo) die("repair needs --case --url --token --repo");
  ...
  const auth = await post(agentUrl(a.url, a.case, "/authorize-repair"), a.token, { repo: name });
  ...
  if (!ok) {
    await post(agentUrl(a.url, a.case, "/repair-confirm"), a.token, {
...
async function cmdDecrypt(a) {
  if (!a.url || !a.token || !a.case || !a.id) die("decrypt needs --case --url --token --id");
  const key = loadKey(a.key);
  if (!key) die("no content key (FORENSIC_CONTENT_KEY or --key) — cannot decrypt");
  const { status, data } = await get(agentUrl(a.url, a.case, `/evidence/${encodeURIComponent(a.id)}/sealed`), a.token);
